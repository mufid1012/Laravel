...
<body class="min-h-screen flex flex-col selection:bg-zinc-800 selection:text-white">
    ...
    <!-- Navigation Header -->
    <header class="border-b border-zinc-900 bg-zinc-950/80 backdrop-blur-md sticky top-0 z-50 transition-all duration-300">
        <div class="max-w-6xl mx-auto px-6 py-5 md:h-20 flex flex-col md:flex-row items-center justify-between gap-4 md:gap-0">
            ...
            @php
                ...
                $navIdle = 'text-zinc-400 hover:text-white';
            @endphp

            <nav class="flex flex-wrap items-center justify-center gap-4 md:gap-7">
                @auth
                    @if(auth()->user()->is_admin)
                        <a href="{{ route('admin.dashboard') }}" class="text-xs tracking-widest {{ request()->routeIs('admin.dashboard') ? $navActive : $navIdle }} transition-colors">
                            DASHBOARD ADMIN
                        </a>
                        <a href="{{ route('admin.products.index') }}" class="text-xs tracking-widest {{ request()->routeIs('admin.products.*') ? $navActive : $navIdle }} transition-colors">
                            KATALOG
                        </a>
                        <a href="{{ route('admin.orders.index') }}" class="text-xs tracking-widest {{ request()->routeIs('admin.orders.*') ? $navActive : $navIdle }} transition-colors">
                            ORDER USER
                        </a>
                    @else
                        <a href="{{ route('dashboard') }}" class="text-xs tracking-widest {{ request()->routeIs('dashboard') ? $navActive : $navIdle }} transition-colors">
                            DASHBOARD
                        </a>
                        <a href="{{ route('store') }}" class="text-xs tracking-widest {{ request()->routeIs('store') ? $navActive : $navIdle }} transition-colors">
                            KATALOG
                        </a>
                        <a href="{{ route('orders.history') }}" class="text-xs tracking-widest {{ request()->routeIs('orders.history') ? $navActive : $navIdle }} transition-colors">
                            RIWAYAT
                        </a>
                    @endif
                ...
                @else
                    ...
                    <a href="{{ route('login') }}" class="text-xs tracking-widest {{ request()->routeIs('login') ? $navActive : $navIdle }} transition-colors">
                        LOGIN
                    </a>
